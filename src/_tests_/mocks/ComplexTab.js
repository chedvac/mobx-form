...
export default class ComplexTab extends ComplexType {
    ...
    @modelProp() @formObservable ({ validations:[hebrewName({message: 'hebrew only'}), maxlength({value: 5, message: 'too long...'})]}) agreement = "";        
    @modelProp() @formObservable ({validations:[hebrewName({message: 'hebrew only'}), maxlength({value: 5, message: 'too long...'})]}) firstName = "yael";        
    @modelProp() complex;
};