...
export let modelPropGenerator = function ({ target, name, descriptor, ...params } = params) {
    ...
    target.propertiesManager.setModelProp(name, { setMap: map, reset, ref: descriptor });
}
export default function modelProp(settings = {}) {
    return function (target, name, descriptor) {
        target.registerProperty({ name, descriptor, isModelProp: true, ...settings })
